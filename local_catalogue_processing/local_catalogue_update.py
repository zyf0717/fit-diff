...
def build_file_dataframe(files, base_dir):
    """
    Build a dataframe with filename, relative_path, date, and FIT time bounds.
    """
    date_pattern = re.compile(r"\d{8}")
    df = (
        pd.DataFrame(
            [_build_file_row(path, base_dir, date_pattern) for path in files],
            columns=["filename", "relative_path", "date", "start_time", "end_time"],
        )
...
def _build_file_row(path, base_dir, date_pattern):
    ...
    if filename.lower().endswith(".fit"):
        start_time, end_time = _extract_fit_time_bounds(path)
    elif filename.lower().endswith((".csv", ".xlsx")):
        start_time, end_time = _extract_tabular_time_bounds(path)
    return {
        "filename": filename,
...
if __name__ == "__main__":
    ...
    if is_xlsx and "Time (HH:mm:ss.000)" in df.columns:
        values = (
            df["Time (HH:mm:ss.000)"]
            .astype(str)
            .str.strip()
            .str.replace(r"\s+", " ", regex=True)
        )
        timestamps = _parse_datetime_series(
            values, ["%d/%m/%Y %H:%M:%S", "%d/%m/%Y %H:%M:%S.%f"]
        )
